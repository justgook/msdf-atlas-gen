...
This fork vendors `msdfgen` and `artery-font-format` directly in the repository and does not rely on git submodules.

### Fork maintenance: syncing vendored dependencies

If you want to update vendored dependencies from upstream:

1. Fetch latest upstream `msdf-atlas-gen` and identify target commits/tags for `msdfgen` and `artery-font-format`.
2. Replace local `msdfgen/` and `artery-font-format/` directories with the desired upstream snapshots.
3. Keep fork-specific files intact (for example `wasm/`, `web/`, `Makefile`, and wasm-related patches).
4. Run a full rebuild (`make wasm`) and verify the browser demo (`make serve`) before committing.

This keeps the fork self-contained while still allowing explicit, controlled dependency updates.

## WebAssembly demo (fork addition)

This fork includes a browser-oriented wasm build that generates atlas bytes and JSON directly from uploaded font bytes.

- Build wasm module: `make wasm`
- Run demo server: `make serve`
- Open: `http://localhost:8080`

Files added by this fork:

- `wasm/atlas_wasm.cpp` - C API wrapper for in-memory generation
- `web/index.html` - upload/generate/download browser UI
- `Makefile` - Nix + Emscripten build pipeline
